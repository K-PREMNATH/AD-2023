...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_users_list`()
BEGIN
    select USER_ID,
           FIRST_NAME,
           LAST_NAME,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_login_detail`(IN pUsername varchar(45), IN pPassword varchar(500),
                                                         OUT rUserId int, OUT rLastName varchar(50),
...
-- Dump completed on 2023-07-23 17:46:39
